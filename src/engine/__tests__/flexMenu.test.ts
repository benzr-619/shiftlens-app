...
describe('§2.3 shift-menu flexibility search (bounded, opt-in, advisory)', () => {
  it('holds every axis at the current structure when nothing is enabled (one regularized candidate)', () => {
    const cands = searchFlexibleMenus(threeBlockRequirement(), currentMenu, AXES({}), budget, 0.1, 2.5, 2, 2);
  ...
  it('shiftCount axis explores 2/3/4-shift menus', () => {
    const cands = searchFlexibleMenus(threeBlockRequirement(), currentMenu, AXES({ shiftCount: true }), budget, 0.1, 2.5, 2, 2);
  ...
  it('shiftLengths axis explores 8/10/12-hour shifts', () => {
    const cands = searchFlexibleMenus(threeBlockRequirement(), currentMenu, AXES({ shiftLengths: true }), budget, 0.1, 2.5, 2, 2);
  ...
  it('is bounded and deduped: at most 45 candidates even with every axis on', () => {
    const cands = searchFlexibleMenus(
      threeBlockRequirement(),
      currentMenu,
      AXES({ startTimes: true, shiftCount: true, shiftLengths: true }),
  ...
  it('returns nothing for an empty menu', () => {
    expect(searchFlexibleMenus(threeBlockRequirement(), [], AXES({ shiftLengths: true }), budget, 0.1, 2.5, 2, 2)).toEqual([]);
  });
});
